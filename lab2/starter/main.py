            ...
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) > 1:
                    if len(grid.get_cells()[i][j]) == 2:
                        return (i,j)
                    elif len(grid.get_cells()[i][j]) < smallest:
                        position = (i, j)
                        smallest = len(grid.get_cells()[i][j])
        return position
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while Q:
            ...
            variables_assigned_1, failure_1 = ac3.remove_domain_row(grid, var[0], var[1])
            variables_assigned_2, failure_2 = ac3.remove_domain_column(grid, var[0], var[1])        
            variables_assigned_3, failure_3 = ac3.remove_domain_unit(grid, var[0], var[1])

            if (failure_1 or failure_2 or failure_3):
                return False
            if variables_assigned_1:
                Q.append(variables_assigned_1[0])  
            if variables_assigned_2:
                Q.append(variables_assigned_2[0])
            if variables_assigned_3:
                Q.append(variables_assigned_3[0])
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        for i in grid.get_cells()[var[0]][var[1]]:
            ac3 = AC3()
           
            copy_g = grid.copy()
            copy_g.get_cells()[var[0]][var[1]] = i
            
            if (ac3.consistency(copy_g, [var])):
...
for p in problems:
    ...
    ac3.pre_process_consistency(g2)
    
    start = time.time()
    result_1 = backtracking.search(g, fa)
    if not result_1:
        print("Fail")
    end = time.time()
    FA_time.append(end - start)
    ...
    MRV_time.append(end - start)    
    
plotter = PlotResults()
plotter.plot_results(MRV_time, FA_time,
